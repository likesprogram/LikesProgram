...
namespace LikesProgram {
    namespace Time {
        // 时间模块统一使用 system_clock 时间点和 chrono duration 别名。
        using TimePoint = std::chrono::system_clock::time_point;
        using Duration = std::chrono::nanoseconds;
        using Nanoseconds = std::chrono::nanoseconds;
        // 以下 duration 别名只改变单位，不改变底层 chrono 语义。
        using Microseconds = std::chrono::microseconds;
        using Milliseconds = std::chrono::milliseconds;
        using Seconds = std::chrono::seconds;
        // 分钟以上单位用于业务层表达，不改变 FormatTime 输入类型。
        using Minutes = std::chrono::minutes;
        // 小时和天用于格式化/换算场景，Days 固定为 86400 秒。
        using Hours = std::chrono::hours;
        using Days = std::chrono::duration<int64_t, std::ratio<86400>>;

        // 格式化时间
        LIKESPROGRAM_CORE_API LikesProgram::String FormatTime(const TimePoint& tp, const LikesProgram::String& fmt = u"%Y-%m-%d %H:%M:%S");

        // std::time_t → std::tm
        LIKESPROGRAM_CORE_API std::tm ToLocalTime(std::time_t t);
